import { LocalDevelopBalanceWithdrawn, RescueBalanceWithdrawn, TreejerDevelopBalanceWithdrawn, TreeResearchBalanceWithdrawn, WethFunds as WFcontract, TreeFunded } from "../../generated/WethFunds/WethFunds";
...
import { FinancialModel as FMcontract } from "../../generated/WethFunds/FinancialModel";
...
export function handleLocalDevelopBalanceWithdrawn(event: LocalDevelopBalanceWithdrawn): void {
    ...
    totalFunds.totalBalance = totalFunds.totalBalance.minus(withdraw.amount);
    let wfc = WFcontract.bind(event.address);
    totalFunds.localDevelop = wfc.totalFunds().value1 as BigInt;
    totalFunds.save();
}
...
export function handleRescueBalanceWithdrawn(event: RescueBalanceWithdrawn): void {
    ...
    totalFunds.totalBalance = totalFunds.totalBalance.minus(withdraw.amount);
    let wfc = WFcontract.bind(event.address);
    totalFunds.rescueFund = wfc.totalFunds().value2 as BigInt;
    totalFunds.save();
}
...
export function handleTreejerDevelopBalanceWithdrawn(event: TreejerDevelopBalanceWithdrawn): void {
    ...
    totalFunds.totalBalance = totalFunds.totalBalance.minus(withdraw.amount);
    let wfc = WFcontract.bind(event.address);
    totalFunds.treejerDevelop = wfc.totalFunds().value3 as BigInt;
    totalFunds.save();
}
...
export function handleTreeFunded(event: TreeFunded): void {
    ...
    let dfc = WFcontract.bind(event.address);
